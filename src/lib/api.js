...
async function fetchAPI(
  ...
  const cachedCall = unstable_cache(
    async (q, v) => {
      executed = true; // inner ran => MISS
      // if (!isProd) {
      //   console.log(`[Contentful] 💾 CACHE MISS (key=${key})`, {
      //     variables: v,
      //     tags,
      //   });
      // }
  ...
  if (!executed) {
    // inner did not run => HIT
    console.log(`[DATA] ✅ CACHE HIT  (key=${key})`, { variables, tags });
  }
