      ...
      <div className="container">
        <div className="row">
          {arrays.map(({ letter, items }, groupIndex) => (
            <div
              className="col-md-4 clientGrid--item"
              key={`group-${groupIndex}`}
            >
              <div
                className={`${alphabet.clients_alpha_cont} ${style.clients_alpha_cont}`}
              >
                <h2 className={alphabet.alpha_letter}>{letter}</h2>
                {items.map((client, clientIndex) => {
                  const projectData = client?.projects?.nodes || [];
                  const leadImgSrc =
                    projectData[0]?.featuredImage?.node?.sourceUrl ||
                    loader.src;
                ...
                <span
                  className={`${alphabet.client_white_bg} ${alphabet.client_white_bg_1}`}
                />
                <span
                  className={`${alphabet.client_white_bg} ${alphabet.client_white_bg_2}`}
                />
                <span
                  className={`${alphabet.client_white_bg} ${alphabet.client_white_bg_3}`}
                />
                <span
                  className={`${alphabet.client_white_bg} ${alphabet.client_white_bg_4}`}
                />
              </div>
            </div>
          ))}
