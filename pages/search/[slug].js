...
export default function search({ data, filter }) {
    ...
    if (filter?.clients?.edges != '') {
        ...
        if (filter?.clients?.edges[0].node.projects.edges != '') {
            ...
            var clientData = clientsprojects.edges.map(({ node }) => {
                ...
                return (
                    <>
                        <div className={`col-md-4 project__item resultItem-cont ${node?.id}`} key={node.id}>
                                    ...
                                    <a href={`/projects/${node.slug}`}>
                                        <span className={`${carousel.projectThumbnail} fade-in`} style={{ "width": "100%" }}>
                                            <div className={`${carousel.full_thumb} full-thumb`}>
                                                {leadImgSrc && (
                                                    <Image priority={true} placeholder="blur" blurDataURL={`data:image/svg+xml;base64,${toBase64(shimmer(700, 475))}`} className={carousel.project_lead} src={leadImgSrc} alt="project-lead"
                                                        layout="fill" />
                                                )}
                                            </div>
    ...
    if (filter?.categories?.edges != '') {
        if (filter?.categories?.edges[0].node.posts?.edges != '') {
            ...
            var allposts = postsCat.edges.map((data) => {
                ...
                return (
                    <div className={`col-md-4 project__item resultItem-cont ${data?.node?.id}`} key={data?.node?.id}>
    ...
    //     filter?.projects?.edges?.map(({ node }) => {
    //         psID.push(node?.id)
    //     })
    // }

    // var allPrId = [...cID, ...IID, ...kID, ...ptID]

    // var filterID = [...new Set(allPrId)]
    ...
    // var pR = 0
    // if (filter?.projects?.edges != '') {
    //     var clientsprojects = filter?.projects?.edges
    //     pR = clientsprojects.length
    //     var projectData = clientsprojects?.map(({ node }, i) => {
    //         var leadImgSrc = node?.featuredImage?.node?.sourceUrl
    //         if (filterID[i] === psID[i]) {
    //             return (
    //                 <>
    //                     <div className={`col-md-4 project__item resultItem-cont ${node?.id}`} key={node.id}>
    //                         <div className={`${carousel.projectCarousel} ${type.projectCarousel}`}>
    ...
    //     var poststags = filter?.posts?.edges
    //     pD = poststags?.length
    //     var allpost = poststags.map((data) => {
    //         var categories = data?.node?.categories?.edges[0]?.node?.name
    //         var featuredImage = data?.node?.featuredImage?.node?.sourceUrl
