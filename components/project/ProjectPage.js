...
export default function ProjectPage({ project, oProjects }) {
  ...
  const other_projects_slider = other_projects?.map(({ node }) => {
  ...
  const leadComponentMobile = project?.projectComponent?.leadComponentMobile?.sourceUrl;
  ...
  const relatedProjects_slider = relatedProjects?.map((node) => {
    const leadImgSrc = node?.featuredImage?.node?.sourceUrl;
    const client = node?.clients?.edges[0]?.node.name;
  ...
  const awardImgMobile = project?.projectComponent?.awardsImageMobile && (
    <Image
      ...
      src={project?.projectComponent?.awardsImageMobile?.sourceUrl || 'https://digital.icdindia.com/wp-content/uploads/2023/04/Jack-Daniels-Gentleman-Jack-Components2-2048x1280.jpg'}
      alt="project-lead"
      layout="fill"
      sizes="100vw"
    />
  ...
  const leadVideo = project?.projectComponent?.leadVideo && project?.projectComponent?.video?.mediaItemUrl;
  const leadVideo_mobile = project?.projectComponent?.leadVideo && project?.projectComponent?.videoMobile?.mediaItemUrl;

  const shareBtn = (
    <div className={`${style.social__media} social__media`}>
      <span className={`icon share-icon ${!showShareIcons && 'icons-hide'}`}><a href={linkedinUrl} rel="noopener" aria-label="icd" className="linkedin-icon" target="_blank"></a></span>
      <span className={`icon share-icon ${!showShareIcons && 'icons-hide'}`}><a href={twitterUrl} rel="noopener" aria-label="icd" className="twitter-icon" target="_blank"></a></span>
      <span className={`icon share-icon ${!showShareIcons && 'icons-hide'}`}><a href={fbUrl} rel="noopener" aria-label="icd" className="fb-icon" target="_blank"></a></span>
      <span className="icon" onClick={() => setShowShareIcons(!showShareIcons)}><img loading='lazy' alt='icd' decoding="async" src={Share.src} width="20" height="20" className="icon-img shareIcon--main" />share</span>
  ...
  const SlickArrowRight = ({ currentSlide, slideCount, ...props }) => (
    <button
      {...props}
      className={`slick-next slick-arrow${currentSlide === slideCount - 3 ? " slick-disabled" : ""}`}
      aria-hidden="true"
  ...
  return (
    <>
      ...
      <article className={` ${style.singleProject} ${awards && style.winnerTag} singleProject`}>
        {showModal && (
          <div className={`project_details_modal ${style.project_details_modal}`}>
            <div className="container">
              <div className="row">
                <span className={`close-button ${style.close_button}`} onClick={() => setShowModal(false)}></span>
        ...
        <div className={`container ${style.projectHeading}`}>
          ...
          <div className="row">
            <div className="col-12 offset-md-3 col-md-8 col-xl-8 offset-xl-4">
              ...
              <div className={style.projectMeta__cont}><span className={style.category__title}>client</span><span className={style.category}>{project?.clients?.edges?.map((data, i) => (allclients === i + 1) ? data?.node?.name : `${data?.node?.name}, `)}</span></div>
              {projectLink && (
            ...
            <div className="col-12 offset-md-3 col-md-8 col-xl-8 offset-xl-4">
              <div className={style.project__summary}>
                <div className={style.intro_para}>
                  <div>
                    {shortDesc && parse(shortDesc)}
                  </div>
                </div>
                <button className={style.collapse__btn} onClick={() => setShowModal(true)}>project detail</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="container">
          <div className="col-12 offset-md-3 col-md-8 col-xl-8 offset-xl-4">
            <div className={style.team__block}>
              ...
              <div className={`team__content ${seeAll === 'see less' && style.ellipsis}`}>
                <div className={style.team__detail}>
                  {team && parse(team)}
                </div>
              </div>
              <a className={style.team__seeAll} rel="noopener" aria-label="icd" onClick={toggleSeeAll}>{seeAll}</a>
              {addDisclaimer && <span className={style.disclaimer}>Fonts and stock images, if used, are for representation purposes. The rights of commercial usage have not been purchased.</span>}
              <button className="collapse__btn" onClick={() => setShowModal(true)}>project detail</button>
            </div>
          </div>
        </div>
          ...
          <div className='container slider-container'>
            <div className={style.more__projects_block}>
              <div className={`more_cont ${style['more_cont']}`}>
                ...
                <span className={`${style.see_all} see-all`}>
                  <Link prefetch={false} href={` /projects/category/${project?.projectTypes.edges[0]?.node?.slug}`}>see all</Link>
                </span>
              </div>
              <span className="bottom__border"></span>
              <div className="more-projectsCarousel">
