...
async function fetchAPI(query, { variables } = {}) {
  ...
  const headers = { 'Content-Type': 'application/json', 'User-Agent': '*', 'Authorization': 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwczpcL1wvZGlnaXRhbC5pY2RpbmRpYS5jb20iLCJpYXQiOjE2ODE0NTI1NjIsIm5iZiI6MTY4MTQ1MjU2MiwiZXhwIjozMzIxNzQ1MjU2MiwiZGF0YSI6eyJ1c2VyIjp7ImlkIjoiMSJ9fX0.esvik6maiGos-5LvzBV3ncjv5M1F9cxb7JPAlXxi9g0' };
...
export async function getAllProjectsNotIn(categorySlug, projectIds) {
  const data = await fetchAPI(
    `
    {
      projectTypes(where: {slug: "${projectIds}"}) {
        edges {
          node {
            projects(where: {notIn: "${categorySlug}"}) {
              edges {
                node {
                  id
                  slug
...
export async function getIndustries() {
  const data = await fetchAPI(
    `
    {
      industries(first: 500, where: {orderby: TERM_ORDER}) {
        nodes {
          name
          slug
          id
...
export async function getTeam() {
  const data = await fetchAPI(
    `
    {
      teams(where: {orderby: {field: MENU_ORDER, order: ASC}},first:25) {
        edges {
          node {
            title
            id
