const API_URL = 'http://localhost/nextjs/graphql'

async function fetchAPI(query, { variables } = {}) {
  // Set up some headers to tell the fetch call
  // that this is an application/json type
  const headers = { 'Content-Type': 'application/json' , 'User-Agent': '*' , 'Authorization' : 'Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpc3MiOiJodHRwOlwvXC9sb2NhbGhvc3RcL25leHRqcyIsImlhdCI6MTYzMjgwNTM4MiwibmJmIjoxNjMyODA1MzgyLCJleHAiOjMzMTY4ODA1MzgyLCJkYXRhIjp7InVzZXIiOnsiaWQiOiIxIn19fQ.m80DnTVd5-9NqOBikgTT3IacfsiIkFYB28OL57k7f9U'};
...
export async function comment(value, value2) {
  const data = await fetchAPI(
    `
    mutation CREATE_COMMENT {
      createComment(input: {commentOn: ${value2}, content: "${value}", author: "admin"}) {
        success
        comment {
          id
          content
...
export async function careerContact(firstName , lastName , enquiryAbout , email , number , linkedin , designation , company , message , companyWebsite) {
  const data = await fetchAPI(
    `
    mutation CREATE_CONTACT {
      careerContactForm(
        input: {enquiryAbout: "", firstName: "", lastName: "", educationalQualification: "", experience: "", number: "", city: "", email: "", website: ""}
      ) {
        data
        success
      }
